...
//			}
			...
			if(redoDAO.findAll(newBean).size() == 0){
				 ...
				 * 增加执行时间
				 */
				setExecTime(syncBean, (Long) event.getContext().getParameters().getValue("flowStartTime"));
	...
	public void insertRedo(ErrorEvent event) {
		try {
			...
			String syncId = document.getContext().getId();
			...
			//更新执行时间，异常也需要记录执行时间
			SyncBean syncBean = syncDAO.findById(syncId);
			if (syncBean!=null){
				setExecTime(syncBean, (Long) event.getDocument().getContext().getParameters().getValue("flowStartTime"));
				syncDAO.update(syncBean);
			}
	...
	private void setExecTime(SyncBean syncBean,long startTime){
		long endTime = System.currentTimeMillis();
		long execTime = endTime-startTime;
		syncBean.setExec_time(execTime);
	}
